...
(define-twitter-method mentions (() &key (since-id nil) (max-id nil) (count nil) ;;(page nil)
				 (trim-user nil) (include_rts nil) (include-entities t)) :statuses/mentions)
(define-twitter-method retweeted-by-me (() &key (since-id nil) (max-id nil) (count nil);; (page nil)
					(trim-user nil) (include_rts nil) (include-entities t)) :statuses/retweeted-by-me)

(define-twitter-method retweeted-to-me (() &key (since-id nil) (max-id nil) (count nil) ;;(page nil)
					(trim-user nil) (include_rts nil) (include-entities t)) :statuses/retweeted-to-me)

(define-twitter-method retweets-of-me (() &key (since-id nil) (max-id nil) (count nil) ;;(page nil)
				       (trim-user nil) (include_rts nil) (include-entities t)) :statuses/retweets-of-me)


;;----------------------------------------------------------------------------------------------------------------------------------------------------

(defun collect-home-timeline (&key (count 5) (max 3) (skip 0)
				(container (make-hash-table  :test 'equal :size 100)))
  (collect-tweets (:count count :max max :skip skip :container container) home-timeline))

;;;; 2016-07-22 José Miguel Ronquillo RIvera; este end-point ya no existe.
#+nil
(defun collect-friends-timeline (&key (max 3) (skip 0) (container (make-hash-table  :test 'equal :size 100)))
  (collect-tweets (:max max :skip skip :container container :type :friends) friends-timeline))

(defun collect-user-timeline (&key (count 15) (max 3) (skip 0)
				(container (make-hash-table  :test 'equal :size 100)))
  (collect-tweets (:count count :max max :skip skip :container container :type :user-timeline) user-timeline))
